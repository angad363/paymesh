 ...
 * generator can claim it in a separate transaction. Each test uses its own generated merchant id.
 *
 * <p>ADR-043: this deployable carries no {@code MerchantStatusGate}/{@code merchant_ref} (neither
 * Reporting, Notification nor Audit ever imported them), so unlike the monolith's version of this
 * test, a report-export write here is not blocked by a merchant's status or refused with 503 for an
 * absent projection row -- a random, never-persisted {@link MerchantId} is enough.
 */
@SpringBootTest
