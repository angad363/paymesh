...
public final class KafkaEventListener {
    ...
    /**
     * THIS DEPLOYABLE'S OWN CONSUMER GROUP (ADR-043, generalizing ADR-042 section 5), not the
     * monolith's {@code paymesh-monolith}. Engagement (Notification + Reporting + Audit) is now a
     * genuinely independent reader of the event stream. Stable by choice, same reason as the
     * monolith's and webhook's: a rename replays every topic from {@code earliest}, safe (the inbox
     * dedupes) but wasteful.
     */
    static final String CONSUMER_GROUP = "paymesh-engagement";
