...
**PostgreSQL + Flyway, twenty-three migrations (V1–V23).** Every capability's `application` layer declares repository interfaces; `infrastructure/persistence/jpa` implements them with a **separate JPA entity, never the domain type**, and a mapper between the two. `ddl-auto=validate`, so a mapped column that drifts from its migration fails startup rather than surprising someone later.
