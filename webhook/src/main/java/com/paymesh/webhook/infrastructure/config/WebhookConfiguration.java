 ...
 * <p>
 * {@code @EnableScheduling} lives here rather than in a platform configuration -- unlike the
 * monolith, this deployable has exactly one capability, and {@code OrderConfiguration}'s "declare it
 * once" reasoning still applies with one candidate instead of many (the same call
 * {@code provider-sim}'s {@code SimulatorConfiguration} made, ADR-041).
 */
@Configuration
...
public class WebhookConfiguration {
    ...
    RotateWebhookSecretService rotateWebhookSecretService(
        ...
        return new RotateWebhookSecretService(
            endpoints, verifiedMasterKey(properties), outboxWriter, transactions, clock
        );
    }
