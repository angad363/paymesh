-- ============================================================================
--  V1__create_provider_simulator.sql
--  The Provider Simulator's own tables. SDD section 13.4, design section 2.
--
--  THIS MODULE'S OWN FLYWAY HISTORY, SEPARATE FROM THE MONOLITH'S (ADR-041).
--  These five tables already exist in the shared cluster's `simulator` schema --
--  the monolith created them as its own V13, back when this module still ran
--  in-process (ADR-038's schema-per-service carve moved them there ahead of
--  extraction). This file is byte-for-byte that migration's DDL, so a FRESH
--  database (a new environment, a Testcontainers run) gets the same schema
...
--  version instead of re-running the CREATE TABLE statements against a schema
--  that already has them -- see the flyway config for why V1 is exactly the
--  baseline version.
--
...
-- ============================================================================
--  EVERYTHING BELOW THIS LINE IS FROM LATER MONOLITH MIGRATIONS, FOLDED IN
--  (ADR-041).
--
--  V13 above is a byte-for-byte copy of the monolith's original migration, but
--  three later ones altered these same five tables before extraction: V22
--  (a refund's own callback reference), V26 (the is_prefixed_id format
--  constraints), and V31 (payouts). Against the shared dev database this whole
--  file is skipped by baseline-on-migrate (the tables already reflect all four
--  migrations), so this section only ever runs for a genuinely fresh database
--  -- which is also the only place it is verified: a Testcontainers run has no
--  V13/V22/V26/V31 history to adopt, so it must reach the exact same end state
--  by executing every statement below for real.
-- ============================================================================

-- --- V22: a simulated refund gets the caller's own reference too (ADR-026) ---
ALTER TABLE provider_refunds
    ADD COLUMN callback_reference VARCHAR(120);

-- --- V26: the shared identifier-format predicate --------------------------
...
-- `simulator`) because it is a platform-wide predicate several schemas rely
...
-- cluster with the monolith) this is a same-definition no-op; on a fresh one
-- it is the only place this module defines it.
...
-- --- V31: payouts (SDD 13.4) -------------------------------------------------
-- The simulator's side of a settlement payout. Deterministic on the
-- destination, exactly as a payment is deterministic on its token.
CREATE TABLE provider_payouts (
    provider_payout_id  VARCHAR(60)              NOT NULL,

    -- PayMesh's payout id, and the idempotency key of this table: a resubmitted
    -- payout returns the row it created the first time rather than moving the
    -- money twice.
    ...
    -- The provider's own vocabulary, not PayMesh's. Translated to PayMesh's
    -- PAID/FAILED at the callback boundary, the only place the two dictionaries
    -- meet.
...
-- One queue, two kinds of callback, rather than a second queue duplicating the
-- dispatcher, the retry budget, the signing and the stored-bytes rule.
-- provider_payment_id becomes nullable; the XOR check below replaces the old
-- guarantee ("every row names a payment") with a stronger one -- every row
-- names EXACTLY ONE of a payment or a payout.
...
-- --- V26 (continued): the remaining is_prefixed_id constraints on this schema
ALTER TABLE provider_outbound_callbacks
    ADD CONSTRAINT ck_provider_outbound_callbacks_payment_id_format
        CHECK (is_prefixed_id(provider_payment_id, 'sim_pay_'));

ALTER TABLE provider_payments
    ADD CONSTRAINT ck_provider_payments_id_format
        CHECK (is_prefixed_id(provider_payment_id, 'sim_pay_'));

ALTER TABLE provider_refunds
    ADD CONSTRAINT ck_provider_refunds_payment_id_format
        CHECK (is_prefixed_id(provider_payment_id, 'sim_pay_'));

ALTER TABLE provider_refunds
    ADD CONSTRAINT ck_provider_refunds_id_format
        CHECK (is_prefixed_id(provider_refund_id, 'sim_ref_'));

ALTER TABLE provider_outbound_callbacks
    ADD CONSTRAINT ck_provider_outbound_callbacks_id_format
        CHECK (is_prefixed_id(outbound_callback_id, 'sim_cb_'));
