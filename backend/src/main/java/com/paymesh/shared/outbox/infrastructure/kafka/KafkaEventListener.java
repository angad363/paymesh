...
public final class KafkaEventListener {
    ...
    /**
     * The monolith's single consumer group. Stable by choice: a rename replays every topic from
     * {@code earliest} (ADR-036) — safe, because the inbox dedupes the replay, but wasteful, so it is
     * fixed once. Each capability gets its OWN group when it is extracted; this is the transitional
     * single reader.
     */
    static final String CONSUMER_GROUP = "paymesh-monolith";
