...
class RefundConfigurationTest {
     ...
     * <p>
     * Notification joined Ledger and Payment in ADR-033, Reporting after it -- neither is a consumer
     * here any more (ADR-043, PR 8): extracted to the engagement deployable together with Audit,
     * their {@code notification.refund.succeeded}/{@code reporting.refund.succeeded} handlers now
     * live in that process, fed by its own {@code paymesh-engagement} Kafka consumer group. Webhook
     * left the same way one PR earlier (ADR-042, PR 7).
     */
    @Test
    void subscribesTheLedgerAndPaymentToRefundSucceeded() {
